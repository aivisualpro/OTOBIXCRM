...
function addKey() {
  ...
  if (!key || valueKeys.value.includes(key))
    return
...
// ─── Helpers ───
function _timeAgo(value?: string): string {
  if (!value)
    return '—'
  const now = new Date()
          ...
          <div class="flex items-center justify-between rounded-lg border p-3">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Enable or disable this dropdown
              </p>
            </div>
          ...
          <template v-if="isStringMode">
            <div class="space-y-2">
              ...
              <p class="text-[11px] text-muted-foreground">
                {{ stringValues.filter(v => v.trim()).length }} value(s) defined
              </p>
            </div>
          </template>
