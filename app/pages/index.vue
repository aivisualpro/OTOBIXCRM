...
const displayedWidgets = computed({
  get: () => {
    ...
    let layout = userLayout.value.filter(id => availableWidgets.value.includes(id))
  ...
  <div class="w-full h-full flex flex-col gap-4 overflow-y-auto pb-10">
    ...
    <main class="@container/main flex flex-1 flex-col">
              ...
              <CardContent class="flex-1">
                ...
                <template v-else>
                  ...
                  <div class="space-y-2 overflow-hidden">
                    <p class="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-3">Recently Joined</p>
                    <div v-for="dealer in dealersOverview.recent" :key="dealer.id" class="flex items-center justify-between p-3 px-4 rounded-xl bg-background/50 border hover:bg-accent/50 text-sm transition-all shadow-sm">
                      ...
                      <Badge variant="outline" class="text-[9px] uppercase tracking-widest px-1.5 shrink-0 ml-2"
                        :class="[
                    ...
                    <div v-if="dealersOverview.recent.length === 0" class="text-xs text-muted-foreground/60 text-center py-4 italic">No dealers recorded.</div>
                  </div>
                </template>
              </CardContent>
            </Card>
      ...
      <div v-if="displayedWidgets.length === 0" class="flex flex-col items-center justify-center p-12 py-24 border rounded-xl border-dashed mt-4">
        ...
        <h3 class="text-lg font-medium">No Widgets Displayed</h3>
        <p class="text-sm text-muted-foreground mt-1">Click the Widgets button to add widgets to your dashboard.</p>
      </div>
      </ClientOnly>
    </main>
  </div>
